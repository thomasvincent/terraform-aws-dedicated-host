...
variable "enable_ssm_management" {
  description = "Whether to enable AWS Systems Manager management for the dedicated host."
  type        = bool
  default     = false
}

variable "ssm_activation_tier" {
  description = "The tier of the Systems Manager activation (if ssm management is enabled)."
  type        = string
  default     = "standard"
  validation {
    condition     = contains(["standard", "advanced"], var.ssm_activation_tier)
    error_message = "Valid values are 'standard' or 'advanced'."
  }
}

variable "ssm_document_name" {
  description = "The name of the SSM document to use for host management (if ssm management is enabled)."
  type        = string
  default     = "AWS-ConfigureAWSHost"
}
...
variable "enable_automated_backups" {
  description = "Whether to enable automated backups of instances on the dedicated host."
  type        = bool
  default     = false
}

variable "backup_plan_schedule" {
  description = "The schedule expression for the backup plan (if automated backups are enabled)."
  type        = string
  default     = "cron(0 1 * * ? *)" # Default: Every day at 1:00 AM UTC
  validation {
    condition     = can(regex("^cron\\(([^)]*)\\)$|^rate\\(([^)]*)\\)$", var.backup_plan_schedule))
    error_message = "Backup plan schedule must be a valid cron or rate expression."
  }
}

variable "backup_retention_days" {
  description = "Number of days to retain backups (if automated backups are enabled)."
  type        = number
  default     = 30
  validation {
...
variable "recovery_point_tags" {
  description = "Tags to assign to recovery points (if automated backups are enabled)."
  type        = map(string)
  default     = {}
}
